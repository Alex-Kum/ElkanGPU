...
public:
    ...
    //virtual double pointPointInnerProduct_new(int x1ndx, int x2ndx) const;
    //virtual double pointCenterInnerProduct_new(int xndx, unsigned short cndx) const;
    //virtual double centerCenterInnerProduct_new(unsigned short c1ndx, unsigned short c2ndx) const;
    virtual Dataset const* getCenters() const { return centers; }
    /*
    virtual double pointCenterDist2_new(int x1, unsigned short cndx) const {
        #ifdef COUNT_DISTANCES
        ++numDistances;
        #endif
        //std::cout << x1 << "\n";
        return pointPointInnerProduct_new(x1, x1) - 2 * pointCenterInnerProduct_new(x1, cndx) + centerCenterInnerProduct_new(cndx, cndx);
    }
    */
protected:
    ...
    //virtual void changeAssignment_new(int xIndex, int closestCluster, int threadId);
